...
const GS = () => (
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .orb{position:fixed;border-radius:50%;filter:blur(120px);pointer-events:none;z-index:0}
    .orb1{width:600px;height:600px;background:radial-gradient(circle,rgba(0,255,224,0.06),transparent 70%);top:-200px;right:-100px}
    .orb2{width:500px;height:500px;background:radial-gradient(circle,rgba(139,92,246,0.07),transparent 70%);bottom:-100px;left:-150px}
    .card{background:var(--card);border:1px solid var(--border);border-radius:var(--r);position:relative;overflow:hidden}
    ...
    .btn{display:inline-flex;align-items:center;gap:8px;font-family:var(--fh);font-weight:600;font-size:14px;cursor:pointer;transition:all .2s;border:none;outline:none}
    .btn-neon{background:linear-gradient(135deg,var(--neon2),#00a8f0);color:#000;padding:11px 26px;border-radius:50px}
    .btn-neon:hover{transform:translateY(-2px);box-shadow:0 8px 30px -5px rgba(0,255,224,0.4)}
    .btn-neon:disabled{opacity:0.4;cursor:not-allowed;transform:none;box-shadow:none}
    .btn-outline{background:transparent;border:1px solid var(--border2);color:var(--text2);padding:10px 22px;border-radius:50px}
    .btn-outline:hover{border-color:var(--neon);color:var(--neon);background:var(--neon3)}
    .btn-ghost{background:transparent;border:1px solid var(--border);color:var(--text3);padding:7px 16px;border-radius:var(--rs);font-size:13px}
    .btn-ghost:hover{border-color:var(--border2);color:var(--text2)}
    .btn-danger{background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);color:var(--red);padding:9px 20px;border-radius:50px}
    .inp{background:var(--dark3);border:1px solid var(--border2);color:var(--text);font-family:var(--fh);font-size:14px;border-radius:var(--rs);padding:11px 14px;width:100%;outline:none;transition:border-color .2s,box-shadow .2s}
    .inp:focus{border-color:var(--neon);box-shadow:0 0 0 3px rgba(0,255,224,0.08)}
    .inp::placeholder{color:var(--text4)}
    textarea.inp{resize:vertical;min-height:80px}
    ...
    .badge-card{display:flex;flex-direction:column;align-items:center;gap:8px;padding:18px 12px;border-radius:14px;border:1px solid var(--border);background:var(--card);transition:all .2s}
    ...
    .streak-day{width:30px;height:30px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700}
    ...
    .toast{position:fixed;top:24px;right:24px;z-index:9999;padding:14px 20px;border-radius:14px;background:var(--card2);border:1px solid var(--neon);color:var(--text);font-size:14px;display:flex;align-items:center;gap:10px;box-shadow:0 8px 30px rgba(0,255,224,0.15);animation:fadeUp .3s ease}
    .modal-overlay{position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(10px);z-index:1000;display:flex;align-items:center;justify-content:center;padding:20px}
    .modal{background:var(--dark2);border:1px solid var(--border2);border-radius:24px;padding:36px;width:100%;max-width:460px;position:relative}
    ...
    .lb-row{display:flex;align-items:center;gap:14px;padding:12px 16px;border-radius:12px;border:1px solid var(--border);background:var(--card);margin-bottom:8px;transition:all .2s}
...
function Sidebar({page,setPage,user,onLogout}) {
  return (
    <aside style={{width:230,minHeight:"100vh",background:"linear-gradient(180deg,var(--dark),var(--dark2))",borderRight:"1px solid var(--border)",padding:"0 10px 24px",flexShrink:0,position:"sticky",top:0,height:"100vh",display:"flex",flexDirection:"column",zIndex:10,overflowY:"auto"}}>
      <div style={{padding:"22px 10px 18px",borderBottom:"1px solid var(--border)",marginBottom:10}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontFamily:"var(--fd)",fontSize:18,letterSpacing:".06em",lineHeight:1}}>SKILL<span style={{color:"var(--neon)"}}>BRIDGE</span></div>
            <div style={{fontSize:9,color:"var(--text4)",letterSpacing:".15em",textTransform:"uppercase",marginTop:2}}>Career Intelligence</div>
          </div>
        </div>
      </div>
      ...
      <div style={{marginTop:12,padding:"10px 14px",background:"rgba(0,255,224,0.04)",borderRadius:10,border:"1px solid rgba(0,255,224,0.1)",marginBottom:10}}>
        <div style={{fontSize:11,color:"var(--neon)",fontWeight:600,marginBottom:4}}>🤖 Claude AI Powered</div>
        <div style={{fontSize:10,color:"var(--text3)",lineHeight:1.6}}>Real-time intelligence across all features.</div>
      </div>
...
function LeaderboardPage({user}) {
  ...
  const medals=["🥇","🥈","🥉"];
  return (
    <div style={{padding:"44px 52px",maxWidth:800,position:"relative",zIndex:1}}>
...
function ProfilePage({user,onUpdate}) {
  ...
  return (
    <div style={{padding:"44px 52px",maxWidth:900,position:"relative",zIndex:1}}>
...
export default function App() {
  ...
  function handleXP(badgeId){
    ...
    if(badgeId&&!u.badges.includes(badgeId)){
      ...
      if(badge){
        ...
        addToast(`🏅 Badge Earned: ${badge.icon} ${badge.name} (+${badge.xp} XP)`);
    ...
    if((u.streak||0)>=7&&!u.badges.includes("streak_7")){
      ...
      addToast("🔥 Week Warrior badge earned! (+200 XP)");
    }
  ...
  const pp={user,setPage,onXP:handleXP,onUpdate:handleUpdate};
  const pages={home:()=><HomePage {...pp}/>,dashboard:()=><DashboardPage {...pp}/>, "skill-gap":()=><SkillGapPage {...pp}/>,roadmap:()=><RoadmapPage {...pp}/>,resume:()=><ResumePage {...pp}/>,trends:()=><TrendsPage {...pp}/>,chat:()=><ChatPage {...pp}/>,interview:()=><InterviewPage {...pp}/>,salary:()=><SalaryPage {...pp}/>,badges:()=><BadgesPage {...pp}/>,leaderboard:()=><LeaderboardPage {...pp}/>,profile:()=><ProfilePage {...pp}/>,};
  ...
  return (
    <>
      ...
      <div style={{display:"flex",minHeight:"100vh",position:"relative",zIndex:1}}>
        ...
        <main style={{flex:1,overflowY:"auto",background:"transparent"}}><Page/></main>
      </div>
    </>
  );
}
